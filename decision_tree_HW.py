...
dataset = pd.read_csv('../과제(의사결정트리)/zoo.csv',
...
def InfoGain(data, split_attribute_name, target_name="class"):
    ...
    # add the code: please compute Weighted_Entropy with the information in line 62
    # ...
    # ...

    # Calculate the information gain
    # add the code: please compute the information gain using total_entropy and Weighted_Entropy variables
    # ...
    # ...
    feature_entropy=0
    for i in range(len(vals)):
        nd = pd.DataFrame(data.loc[data[split_attribute_name]==vals[i]])
        a=counts[i]/np.sum(counts)
        b=entropy(nd[target_name])
        feature_entropy+=a*b
...
def train_test_split(dataset):
    ...
    testing_data = dataset.iloc[20:40].reset_index(drop=True)
